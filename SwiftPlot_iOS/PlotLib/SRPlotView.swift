...
@IBDesignable class SRPlotView: UIView {
    ...
    var yTicks: [String] {
        ...
        set {
            ...
            for label in self.graphAxes.yLockLabels {
                let textSize = label.sizeWithAttributes([NSFontAttributeName: UIFont.boldSystemFontOfSize(15)])
...
//        self.wantsLayer = true
        ...
        let bottomMarginConstraint = NSLayoutConstraint(item: self.titleField!, attribute: .BottomMargin, relatedBy: .Equal, toItem: self, attribute: .BottomMargin, multiplier: 1, constant: 0)
...
//        self.layer?.borderWidth = 1
    ...
    convenience init(frame frameRect: CGRect, title: String, seconds: Double, channels: Int, samplingRatae: CGFloat, padding: CGPoint = CGPointZero) {
...
//            return CGPoint(x: graphAxes.position.x - graphAxes.pointsPerUnit.x, y: (self.frame.height / 2) + graphAxes.position.y - 9)
    ...
    func addSegment() -> SRPlotSegment
    {
        ...
        if self.window != nil {
            segment.layer.contentsScale = UIScreen.mainScreen().scale
        }
//        Swift.print(segment.layer.frame, self.axeLayer?.frame, self.axeLayer?.layer.frame, self.axeLayer?.dataLayer.frame)
//        segment.layer.backgroundColor = UIColor.redColor().CGColor
